...
public class Livrogive extends Commands {
	...
	public void perform(CommandSender s, String lb, String[] args) {
        ...
        if (!s.hasPermission(m.getString("Permissões.giveitem"))) {
            ...
            case 3: {
                ...
                if (Helper.isInteger(itemNameOrId)) {
                    ...
                    item = LivroManager.getItem(id);
                } else {
                    item = LivroManager.getItem(itemNameOrId);
                }
                if (item == null) {
        ...
        s.sendMessage(m1.getString("Comando-Errado.Livro").replace("&", "§").replace("{itens}",this.getItemList()));
        return;
    }
	...
	private String getItemList() {
        ...
        LivroManager.getItems().forEach(item -> { StringBuilder stringBuilder2 = builder.append(String.valueOf(item.getName()) + ", "); });
