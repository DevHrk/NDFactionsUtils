...
public class EntityItem extends net.minecraft.server.v1_8_R3.EntityItem implements IItem {
	...
	@Override
	public void update() {
		// Atualize a posição do item para manter ele sincronizado com a linha do
		// holograma
		setLocation(line.getLocation().getX(), line.getLocation().getY(), line.getLocation().getZ());
	}
	...
	public void showToPlayer(Player player) {
		PacketPlayOutSpawnEntity packet = new PacketPlayOutSpawnEntity(this, 2);
		((CraftPlayer) player).getHandle().playerConnection.sendPacket(packet);
	}

	@Override
	public void showToAll() {
		PacketPlayOutSpawnEntity packet = new PacketPlayOutSpawnEntity(this, 2); // 2 é o tipo para EntityItem
	...
	public static class CraftItem extends org.bukkit.craftbukkit.v1_8_R3.entity.CraftItem implements IItem {
		...
		@Override
		public void update() {
			((EntityItem) this.entity).update();
		}

		@Override
		public void hideToPlayer(Player player) {
			((EntityItem) this.entity).hideToPlayer(player);
		}

		@Override
		public void showToPlayer(Player player) {
			((EntityItem) this.entity).showToPlayer(player);
		}

		@Override
		public void showToAll() {
			PacketPlayOutSpawnEntity packet = new PacketPlayOutSpawnEntity((EntityItem) entity, 2); // 2 é o tipo para
																									// EntityItem
			for (Player player : Bukkit.getOnlinePlayers()) {
				((CraftPlayer) player).getHandle().playerConnection.sendPacket(packet);
			}
