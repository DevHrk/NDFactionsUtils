...
public class EntitySlime extends net.minecraft.server.v1_8_R3.EntitySlime implements ISlime {
		...
		if (damageSource instanceof EntityDamageSource) {
			...
			if (entityDamageSource.getEntity() instanceof EntityPlayer) {
				Bukkit.getPluginManager().callEvent(new PlayerInteractEntityEvent(
						((EntityPlayer) entityDamageSource.getEntity()).getBukkitEntity(), getBukkitEntity()));
			}
		}
	...
	@Override
	public void update() {
		// Atualize a posição do slime para manter ele sincronizado com a linha do
		// holograma
	...
	public static class CraftSlime extends org.bukkit.craftbukkit.v1_8_R3.entity.CraftSlime implements ISlime {
		...
		@Override
		public void update() {
			((EntitySlime) this.entity).update();
		}

		@Override
		public void hideToPlayer(Player player) {
			((EntitySlime) this.entity).hideToPlayer(player);
		}

		@Override
		public void showToPlayer(Player player) {
			((EntitySlime) this.entity).showToPlayer(player);
		}

		@Override
		public void showToAll() {
			PacketPlayOutSpawnEntityLiving packet = new PacketPlayOutSpawnEntityLiving((EntitySlime) entity);
			for (Player player : Bukkit.getOnlinePlayers()) {
				((CraftPlayer) player).getHandle().playerConnection.sendPacket(packet);
			}
		}
