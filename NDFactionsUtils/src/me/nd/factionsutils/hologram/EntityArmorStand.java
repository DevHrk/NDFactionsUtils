...
public class EntityArmorStand extends net.minecraft.server.v1_8_R3.EntityArmorStand implements IArmorStand {
	...
	private boolean isWithinRange(EntityPlayer player) {
		...
		return distanceSquared < 8192.0 && player.playerConnection != null;
	}
	...
	public static class CraftArmorStand extends org.bukkit.craftbukkit.v1_8_R3.entity.CraftArmorStand
		...
		@Override
		public void hideToPlayer(Player player) {
			((EntityArmorStand) entity).hideToPlayer(player);
		}

		@Override
		public void showToPlayer(Player player) {
			((EntityArmorStand) entity).showToPlayer(player);
		}
		...
		public void setBodyPose(EulerAngle pose) {
			((EntityArmorStand) entity)
					.setBodyPose(new Vector3f((float) pose.getX(), (float) pose.getY(), (float) pose.getZ()));
		}
		...
		public void setRightArmPose(EulerAngle pose) {
			((EntityArmorStand) entity)
					.setRightArmPose(new Vector3f((float) pose.getX(), (float) pose.getY(), (float) pose.getZ()));
		}
		...
		@Override
		public void update() {
			((EntityArmorStand) entity).update();
		}
		...
		@Override
		public void showToAll() {
			PacketPlayOutSpawnEntityLiving packet = new PacketPlayOutSpawnEntityLiving((EntityArmorStand) entity);
			for (Player player : Bukkit.getOnlinePlayers()) {
				((CraftPlayer) player).getHandle().playerConnection.sendPacket(packet);
			}
		}
